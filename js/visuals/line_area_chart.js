...
function LineAreaChart() {
  ...
  let y = d3.scaleLinear().domain(y_domain).range([height, 0]).nice();
  ...
  let yAxis = d3.axisLeft().scale(y).ticks(8).tickPadding(8);
  ...
  let gradient = defs
    ...
    .attr("x2", "0%")
    .attr("y2", "100%");
  gradient.append("stop").attr("offset", "0%").attr("stop-color", "#ffefa8");
  gradient.append("stop").attr("offset", "100%").attr("stop-color", "#ffd55b");
}
